...
void handleStatusChange(String newStatus) {
  ...
  if (currentStatus == "Drone") {
    ...
    digitalWrite(POLICE_LED_1, HIGH); // THIS IS THE FIX: Turn LEDs on initially
    digitalWrite(POLICE_LED_2, LOW);  // The loop will handle toggling from here
    ledState = HIGH;                 // Sync the blinker state
  } else if (currentStatus == "Ambulance") {
    Serial.println("🚑 Ambulance mode: Blinking LEDs...");
    digitalWrite(AMBULANCE_LED_1, HIGH); // THIS IS THE FIX
    digitalWrite(AMBULANCE_LED_2, HIGH); //
    ledState = HIGH;
  } else if (currentStatus == "Fire") {
    Serial.println("🔥 Fire mode: Blinking LEDs...");
    digitalWrite(FIRE_LED_1, HIGH); // THIS IS THE FIX
    digitalWrite(FIRE_LED_2, LOW);
    ledState = HIGH;
  } else {
    Serial.println("✅ Idle mode. All systems off.");
    // All hardware is already off from the top of the function
  }
}
